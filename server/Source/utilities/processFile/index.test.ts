...
describe("processFile.ts", () => {
	...
	it("should resolve the promise with the component data", () => {
		...
		Object.defineProperty(getComponentType, "default", {
			value: jest.fn(() => "react"),
		});
		Object.defineProperty(generateComponent, "default", {
			value: jest.fn(() => result),
		});
		return processFile(filePath, project).then(
			(component: MezzuriteComponent) => {
	...
	it("should reject the promise when the filePath is null", () => {
		...
		return processFile(filePath, project).catch((error: Error) => {
			expect(error).toEqual(
				new Error("The file path or project cannot be null.")
			);
		});
	});
