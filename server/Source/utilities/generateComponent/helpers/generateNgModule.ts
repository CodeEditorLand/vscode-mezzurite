...
function containsAngularPerfImport(sourceClass: ClassDeclaration): boolean {
	...
	if (ngModuleDecorator != null) {
		const decoratorArgument = ngModuleDecorator
			.getArguments()
			.find((child: Node) => {
...
function containsRoutingServiceStart(sourceClass: ClassDeclaration): boolean {
	...
	if (constructors.length > 0) {
		...
		const constructorParameters = constructors[0].getFirstChildByKind(
			...
			if (constructorParameters != null) {
				if (
					...
					const constructorBlock =
						constructors[0].getFirstChildByKind(SyntaxKind.Block);
					// TODO: Make this checking more robust.
					containsRoutingServiceStart =
						constructorBlock.getChildren().find((child: Node) => {
