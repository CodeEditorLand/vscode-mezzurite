...
describe("treeItemUtilities.ts", () => {
	...
	describe("generateIconPath", () => {
		...
		it("should return null when fulfilled is null", () => {
			expect(
				generateIconPath("componentType", null, "rootPath")
			).toBeNull();
		});
		...
		it("should return the icon path when fulfilled is true", () => {
			expect(generateIconPath("componentType", true, "rootPath")).toBe(
				join("rootPath", "client", "res", "icons", "componentType.svg")
			);
		});

		it("should return the light icon path when fulfilled is false", () => {
			expect(generateIconPath("componentType", false, "rootPath")).toBe(
				join(
					"rootPath",
					"client",
					"res",
					"icons",
					"componentTypeLight.svg"
				)
			);
		});
	});
