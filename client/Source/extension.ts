...
export function activate(context: ExtensionContext) {
	...
	client
		...
		.then(() => {
			client.onNotification(
				"custom/mezzuriteComponents",
				(message: { value: MezzuriteComponent[] }) => {
					treeView = window.createTreeView("mezzuriteComponentList", {
						treeDataProvider: new MezzuriteDataProvider(
							message.value,
	...
	function onTreeViewExpansionEvent(
		event: TreeViewExpansionEvent<MezzuriteTreeItem>,
	): void {
		if (event.element.resourceUri != null) {
			workspace
				.openTextDocument(event.element.resourceUri.path)
